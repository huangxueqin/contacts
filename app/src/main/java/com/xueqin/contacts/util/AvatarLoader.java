...
import com.xueqin.contacts.repository.ContactRepositoryFactory;
...
public class AvatarLoader {
    ...
    public void loadAvatar(@NonNull final ImageView imageView, @NonNull final ContactInfo contactInfo) {
        ...
        if (bitmap != null) {
            ...
            mLoadExecutors.execute(new AvatarLoadTask(viewId, contactInfo));
        }
    }
    ...
    private class AvatarLoadTask implements Runnable {
        ...
        AvatarLoadTask(int viewId, ContactInfo contactInfo) {
            this.viewId = viewId;
        ...
        @Override
        public void run() {
            final Bitmap avatar = ContactRepositoryFactory.defaultRepository(mContext)
                    .loadAvatar(contactInfo);
            mMainHandler.post(new Runnable() {
                @Override
                public void run() {
                    LoadInfo li = mPendingLoadMap.get(viewId);
                    if (li != null && TextUtils.equals(li.key, contactInfo.getAvatarFileName())) {
                        ...
                        if (avatar != null) {
                            mMemoryCache.put(contactInfo.getAvatarFileName(), avatar);
                        }
                    }
                }
            });
